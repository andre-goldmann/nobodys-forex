...
public interface SignalRepository extends ReactiveCrudRepository<SignalEntity, Integer> {
    ...
    @Query(value = "SELECT * from \"Trades\" WHERE (activated IS NULL and tradeid IS NULL and openprice IS NULL) or (activated='' and tradeid = 0 and openprice = 0) or (activated='' and tradeid IS NULL and openprice IS NULL) limit 5")
    Flux<SignalEntity> signalsDev();

    @Query(value = "SELECT * from \"ProdTrades\" WHERE (activated IS NULL and tradeid IS NULL and openprice IS NULL) or (activated='' and tradeid = 0 and openprice = 0) or (activated='' and tradeid IS NULL and openprice IS NULL)")
